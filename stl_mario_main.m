function Attri_FBF = stl_mario_main(playSound)
...
try
    ...
    for i_stage = 1:length(StageList)
        ...
        % Add a single line at the bottom; This is very important to handle out of boundary pixels/collisions
        ...
        camera_step = 1.5;
        camera_pos_x = [1 GAME_RESOLUTION(1)]; % GAME_RESOLUTION = MainAxesSize;
        ...
        while 1
            ...
            while (curTime >= ((CurrentFrameNo) * FRAME_DURATION) && loops < MAX_FRAME_SKIP)
                ...
                if KeyStatus(4)  % If right key is pressed
            ...
            if frame_updated
                ...
                if Mario.Pos(2) > curStageSize(1)   % mario falls out of the frame
                    CloseReq = true;
                    initVariable
                    kill
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                ...
                if Mario.Pos(1) > curStageSize(2)
                    set(prompt_text_handle,'String', {'Ooooops', ...
                        '',...
                        'No Peach Princess Here',...
                        '',...
                        'Sorry', ...
                        'Press ENTER to quit'},'FontSize',16,'Color',[1 1 1],'BackgroundColor',[0 0 1]);
                    drawnow;
                    terminateFlag = true;
                    if KeyStatus(7) % if return is pressed
                        CloseReq = true;
                        initVariable
                        kill
                        return
...
%% Initializations
    function initVariable()
        ...
        Mario.VY_MAX = [4 4 5];   % How "high" Mario can jump
        Mario.VY_MAX_DOWN = 4;
