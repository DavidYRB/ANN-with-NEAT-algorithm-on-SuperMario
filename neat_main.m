...
maxgeneration = 200;
load_flag = 0; 
% 1 --> load record from neatsave.mat at start of algorithm.
% 0 --> algorithm will start with initial record, at generation=1 (default option)

save_flag = 1; 
% 1 --> save record to neatsave.mat at every generation (default option)

% the settings above (0,1) will start with initial population and save all important structures at every generation
% the settings above (1,1) will continue where you have left off.
% Note: other parameters of NEAT should remain the same!!!
...
% matrix will be 4 rows by (number of generations existent) columns, 
% will contain (from top to bottom) 
% 1:number of generation
% 2:mean raw fitness
% 3:max raw fitness
% 4:index of individual in population which has produced max raw fitness
...
crossover.probability_interspecies=0.0 ; % this probability governs the intra/interspecies parent composition being used.
crossover.probability_multipoint=0.6;
% standard-crossover in which matching connection genes are inherited randomly from both parents.
% In the (1-crossover.probability_multipoint) cases, weights of the new connection genes are the mean of the corresponding parent genes
...
mutation.probability_recurrency=0.0; 
% if we are in add_connection_mutation, this governs if a recurrent connection is allowed.
% Note: this will only activate if the random connection is a recurrent one, otherwise the connection is simply accepted. 
% If no possible non-recurrent connections exist for the current node genes, then for e.g. a probability of 0.1, 9 times out of 10 no connection is added.
